...
class AadhaarOCRService implements IOCRService {
  private markerExistsRegex =
    /(Date of Birth|DOB|Government of India|Male|Female)/i;
  private addressExistsRegex = /Address:/i;

  async extractTextFromImage(path: string) {
    try {
      const {
        data: { text },
  ...
  extractAadhaarDetails(frontText: string, backText: string) {
    ...
    // Validating aadhar
    let frontHasMarkers = this.markerExistsRegex.test(cleanFrontText);
    let frontHasAddress = this.addressExistsRegex.test(cleanFrontText);

    let backHasMarkers = this.markerExistsRegex.test(cleanBackText);
    let backHasAddress = this.addressExistsRegex.test(cleanBackText);
...
export const aadharOCRService = new AadhaarOCRService();
